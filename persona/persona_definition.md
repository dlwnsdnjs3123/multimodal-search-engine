...
구현에서 공통으로 참조해야 하는 키와 세션 관심도 매핑은 `persona/persona_registry.json`을 기준으로 관리한다.
서비스별 런타임 제약 때문에 API Gateway와 rec-models 내부에도 얇은 registry 모듈이 있지만, 키 목록과 기본 매핑은 이 파일과 일치해야 한다.
